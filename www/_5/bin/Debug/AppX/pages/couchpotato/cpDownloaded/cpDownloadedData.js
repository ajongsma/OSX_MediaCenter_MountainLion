(function () {
    ...
    function regenerateCPDataDownloaded() {
        // redefine list
        list.splice(0, list.length);
        // add new items to list
        loadCPDataDownloaded();
    }
    regenerateCPDataDownloaded();

    WinJS.Namespace.define("CPDataDownloaded", {
        items: groupedItems,
        ...
        regenerate: regenerateCPDataDownloaded
    });
    ...
    function generateCPDownloaded() {

        var url = "http://" + appData.values["cpIP"] + ":" + appData.values["cpPort"] + "/api/" + appData.values["cpAPI"] + "/movie.list/?status=done";
        return WinJS.xhr({
            url: url,
